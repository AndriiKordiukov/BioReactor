...
// LOADING CONTENT
nutrientId = await loadingMineral(); // Load Vitamin by Name
...
async function loadingMineral() {

  let responseData = await fetchAPI(minerals + byName + `${nutrientName}`);
  nutrientId = await responseData.id;
  rda = await responseData.rda;
  nutrientObject = await responseData;
...
function toggleFormDescription(forms) { // TODO: Implement form description toggle functionality
